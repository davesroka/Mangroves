...
import com.qfi.mangroves.util.ReportsApiUtils;
...
public class ReportsHttpClient extends MainHttpClient {
	...
	private static String PENDING_PHOTO_DIRECTORY = "pending";
	private static String PENDING_VIDEO_DIRECTORY = "pendingvideos";
			...
			if (params != null) {
				...
				if (params.get("videofilenames") != null) {
					if (!TextUtils.isEmpty(params.get("videofilenames"))) {
						...
						for (String filename : filenames) {
							if (ImageManager.getPhotoPath(context, String.format("%s/%s", PENDING_VIDEO_DIRECTORY, filename)) != null) {
								File file = new File(ImageManager.getPhotoPath(context, String.format("%s/%s", PENDING_VIDEO_DIRECTORY, filename)));
	...
	private String uploadVideo(File file) {
		...
		if (TextUtils.isEmpty(youTubeAuthToken)) {
			youTubeAuthToken = youTubeAuth();
		}

		getUploadToken();
		if (!TextUtils.isEmpty(uploadToken) && !TextUtils.isEmpty(uploadURL)) {
			Map<String, AbstractContentBody> parts = new HashMap<String, AbstractContentBody>();
			try {
				parts.put("token", new StringBody(uploadToken));
				parts.put("file", new FileBody(file, "video/mp4"));

				String response = sendMultipartPost(uploadURL, new HashMap<String, String>(), parts);
	...
	private static String sendMultipartPost(String url, Map<String, String> headers, Map<String, AbstractContentBody> parts) {
