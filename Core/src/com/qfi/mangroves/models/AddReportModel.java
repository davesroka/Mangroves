 ...
 **
 **/
package com.qfi.mangroves.models;

import java.io.File;
import java.util.List;
import java.util.Vector;

import com.qfi.mangroves.database.Database;
import com.qfi.mangroves.database.IMediaSchema;
import com.qfi.mangroves.entities.Media;
import com.qfi.mangroves.entities.Photo;
import com.qfi.mangroves.entities.Report;
			...
			if (pendingVideos != null && pendingVideos.length > 0) {
				for (File file : pendingVideos) {
					if (file.exists()) {
						...
						// get report ID;
						media.setReportId(id);
						media.setType(IMediaSchema.VIDEO);
						Database.mMediaDao.addMedia(media);
					}
				}
			}
